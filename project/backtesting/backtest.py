...
from project.backtesting.interactive import BLUE, ORANGE, fprint
from project.binance import BinanceAPIManager, BinanceOrderBalanceManager
from project.binance_ws import BinanceCache, BinanceOrder
from project.config import CONFIG
from project.database import Database, LogScout
from project.logger import DummyLogger
from project.strategies import get_strategy
...
class MockDatabase(Database):
    ...
    def __init__(self, logger: DummyLogger, config: Annotated[EasyDict, CONFIG]):
...
class MockBinanceManager(BinanceAPIManager):
    ...
    def set_reinit_trader_callback(self, reinit_trader_callback: Callable[..., T]):
        self.reinit_trader_callback = reinit_trader_callback  # type: ignore
    ...
    def buy_alt(self, origin_coin: str, target_coin: str, buy_price: float):
        ...
        with self.contextmanager.hidden():
            fprint(
                f"Bought {origin_symbol}",
                f"balance now: {self.balances[origin_symbol]:.2f} {origin_symbol} ",
                BLUE,
            )
        return BinanceOrder(
            defaultdict(
    ...
    def sell_alt(self, origin_coin: str, target_coin: str, sell_price: float):
        ...
        with self.contextmanager.hidden():
            fprint(
                f"Sold {origin_symbol}",
                f"balance now: {self.balances[target_symbol]} {target_symbol}",
                ORANGE,
            )
        return BinanceOrder(
            defaultdict(
...
def backtest(
    ...
    if strategy is None:
        contextmanager.fail(f"Invalid strategy: {CONFIG.STRATEGY}")
        return manager
    ...
    try:
        while manager.datetime < end_date:
            try:
                ...
                contextmanager.fail(f"An error occured\n\n{traceback.format_exc()}")
